...
This window gives you the power to add new task to a file with ease. Since various plugins have their own way of adding properties/[metadata](../Features/MetadataFormats.md) to the task, this window helps you to add the task content by following a global format and formatting all your tasks to follow the same format as configured by you in the setting.

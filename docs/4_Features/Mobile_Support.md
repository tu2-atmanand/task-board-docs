...
- **Board switcher panel**: A new panel has been implemented which can be opened using the hamburger icon on top-right corner, left to the current board name.

### Platform-Specific Features

- **Height Adjustment**: Task Board adjusts its height based on the platform and window dimensions (v1.6.11)
...
5. **Task card menu**: As drag and drop functionality is not available for mobile devices and as its also very difficult to use on small screens. Cards has been provided with a menu, which can be opened by long-pressing the card, or by tapping in the top-right corner within the card.
